...
def main():
    ...
    for cp in cp_list: 
        ...
        relevance = pd.read_csv(cfg.output_folder + '/' + os.path.basename(cfg.dataset_file).replace('.csv','/') + os.path.basename(cfg.dataset_file).replace('.csv', '_{}_{:04d}_{}_datasheet.csv'.format(cv_fold, cp, fold_type)), delimiter=',', header=0, index_col=0)
