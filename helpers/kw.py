        ...
        for dataset in l:
            ...
            with open('{}kw/{}_{}_selection.txt'.format(out_fold, fold+1, dataset[1]), 'w') as sf:
                for feat in kw_features[0:cfg.n_selection]:
                    sf.write(feat + '\n')
            plot_pca.plot(df, features=kw_features[0:cfg.n_selection], norm=cfg.standardized, rescale=cfg.rescaled, class_label=cfg.class_label, colors=cfg.class_colors, file_name=kw_datafile.replace('.csv', '_{}.png'.format(cfg.viz_method)), method=cfg.viz_method, perplexity=cfg.perplexity, n_iter=cfg.n_iter)
