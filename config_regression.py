...
dataset_file   = "DATA/syn/regression_4in100_1000.csv" # path to data file (must be .csv, features as columns, first row and first column are labels, first column after labels should contain the classes or target values)
...
train_epochs        = 250   # training epochs for training the neural networks
...
k                   = 0     # number of folds for stratified k-fold cross-validation, if k <= 1 there is no partition and will use all samples
